    ...
    >
      ...
      >
        {BOARD.map((sq) => {
          ...
          const owner = prop
            ? players.find((p) => p.id === prop.owner_id)
            : null;
          const squareBg = SQUARE_BG[sq.type] ?? "bg-white";
          const isCurrentSquare = players.some(
            (p) => p.position === sq.id && p.id === state.current_player_id,
          );
      ...
      {players
        ...
        .map((player) => {
          ...
          const sameSquare = players.filter(
            (p2) => p2.position === player.position && !p2.is_bankrupt,
          );
